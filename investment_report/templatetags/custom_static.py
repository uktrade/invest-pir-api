...
def custom_static(context, format_string):
    if settings.AWS_ACCESS_KEY_ID is None:
        if 'local' in context and context['local']:
            return 'file://' + \
                os.path.join(settings.STATIC_ROOT, format_string)
...
def custom_media(context, file_field):
    if file_field:
        if settings.AWS_ACCESS_KEY_ID is None:
            if 'local' in context and context['local'] and hasattr(
                    file_field, 'path'):
...
def is_report_availible(reports, lang, market, sector):
    detail_url = reverse(
        'reportadmin:validation_table_detail', args=(
            lang, market, sector,))
